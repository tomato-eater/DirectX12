...
bool SquarePoly::Create(Device& devi, bool amo)
{
	//頂点データ
	{
		Vertex squareVertices[] =
		{
			{{-0.3f,  0.3f,  0.0f}, {0.0f, 1.0f, 1.0f, 1.0f}},
			{{ 0.3f,  0.3f,  0.0f}, {0.0f, 1.0f, 1.0f, 1.0f}},
			{{-0.3f, -0.3f,  0.0f}, {0.0f, 1.0f, 1.0f, 1.0f}},
			{{ 0.3f, -0.3f,  0.0f}, {0.0f, 1.0f, 1.0f, 1.0f}}
		};
		if (amo)
		{
			squareVertices[0].position = { -0.1f,  0.1f,  0.0f };
			squareVertices[1].position = {  0.1f,  0.1f,  0.0f };
			squareVertices[2].position = { -0.1f, -0.1f,  0.0f };
			squareVertices[3].position = { 0.1f, -0.1f,  0.0f };
		}
		auto size = sizeof(squareVertices);
		...
		memcpy_s(data, size, squareVertices, size);
	...
	//インデックスデータ
	{
		unsigned short squareIndices[] = { 0, 1, 2,	3 };
		auto size = sizeof(squareIndices);
		...
		memcpy_s(data, size, squareIndices, size);
...
void SquarePoly::Draw(ComLis& comLis)
{
	...
	comLis.GetList()->DrawIndexedInstanced(4, 1, 0, 0, 0);
}