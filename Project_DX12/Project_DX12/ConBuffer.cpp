...
bool ConBuffer::Create(Device& devi, PosPro& posPro, UINT bufferSize, UINT idx)
{
	auto size = (bufferSize + 255) & ~255;
	...
	auto srvHeap = posPro.GetSrvHeap();
	auto hType = srvHeap->GetDesc().Type;
	if (hType != D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)
	{
		assert(false && "ディスクリプタヒープタイプが違う");
		return true;
	...
	UINT cbvSize = devi.GetDev()->GetDescriptorHandleIncrementSize(hType);

	D3D12_CPU_DESCRIPTOR_HANDLE cpuHand = posPro.GetSrvHeap()->GetCPUDescriptorHandleForHeapStart();
	cpuHand.ptr += idx * cbvSize;
	devi.GetDev()->CreateConstantBufferView(&cbvDesc, cpuHand);

	gpuHandle = posPro.GetSrvHeap()->GetGPUDescriptorHandleForHeapStart();
	gpuHandle.ptr += idx * cbvSize;
